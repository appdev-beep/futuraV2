// src/pages/Supervisor/StartCLPage.jsx
              ...
              <div className="px-6 py-4 overflow-y-auto flex-1 space-y-4">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                  ...
                  {!historyLoading && clHistory.length > 0 && (
                    <div className="overflow-x-auto">
                      ...
                      <p className="text-[11px] text-slate-500 mt-2 italic">💡 Click on any row to view full details</p>
                    </div>
                  )}
                </div>
                ...
                {competencies.length > 0 && (
                  <div className="bg-white border border-slate-200 rounded-lg p-3">
                      ...
                      <table className="w-full text-xs border border-slate-200 rounded-md overflow-hidden">
                        <thead className="bg-slate-100 uppercase text-[11px] text-slate-700">
                          <tr>
                            ...
                            <th className="px-2 py-1 text-left min-w-[200px]">Comments (Justification / Trainings / Certificates, Etc)</th>
                        ...
                        <tbody className="bg-white">
                          {competencies.map((c, i) => (
                            <tr key={c.competency_id} className="border-t border-slate-100">
                              ...
                              <td className="px-2 py-1 text-slate-700">{c.mplr}</td>
                              ...
                              <td className="px-2 py-1 font-semibold text-blue-600">{Number(c.final_score || 0).toFixed(2)}</td>
                    ...
                    <p className="mt-3 text-xs text-slate-700"><strong>Total Final Score:</strong> {totalFinalScore.toFixed(2)}</p>
                  </div>
                )}
                ...
                {competencies.length > 0 && (
                  <div className="bg-white border border-slate-200 rounded-lg p-3">
                    <label className="block text-xs font-medium mb-1 text-slate-700">Remarks for Next Reviewer</label>
                    <textarea
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative z-50 bg-white rounded-lg shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
                <h3 className="text-lg font-semibold text-gray-800">Competency Leveling Details {selectedCLDetails?.id ? `(CL #${selectedCLDetails.id})` : ''}</h3>
                <button onClick={closeCLDetailsModal} className="text-gray-400 hover:text-gray-600 transition text-2xl leading-none">×</button>
              </div>

              {/* Body */}
              <div className="flex-1 overflow-y-auto p-6">
                {clDetailsLoading && (
                  <div className="text-center py-8 text-gray-500">Loading CL details...</div>
                )}
                            ...
                            <tbody className="divide-y divide-gray-200">
                              {selectedCLDetails.items.map((item, idx) => (
                                <tr key={idx} className="hover:bg-gray-50">
                                  ...
                                  <td className="px-4 py-3 text-center">{item.pdf_path ? (<a href={`${import.meta.env.VITE_API_BASE_URL}${item.pdf_path}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline text-xs">View</a>) : (<span className="text-gray-400 text-xs">-</span>)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end">
                <button onClick={closeCLDetailsModal} className="px-4 py-2 text-sm rounded-md bg-gray-600 text-white hover:bg-gray-700 transition">Close</button>
              </div>
            </div>
          </div>
        )}
